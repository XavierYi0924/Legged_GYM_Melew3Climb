...
class Melew3Flat(LeggedRobot):
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        # 🔥 新策略：分段奖励（基础线性+高处指数）
        cfg.rewards.scales.climb_high = 4.0  # 提高权重，让高度奖励更明显
        
        cfg.rewards.scales.orientation = -10.5  # 保持一定约束
        ...
        cfg.rewards.scales.yaw = -0.4
        cfg.rewards.scales.base_height = 0.5
        cfg.rewards.scales.leg_symmetry = -0.4
        cfg.rewards.scales.z_velocity_positive = 2.0  # 适当提高，配合高度奖励
        cfg.rewards.scales.movement_penalty = -100.0  # 降低惩罚（从-100）
        cfg.rewards.scales.horizontal_velocity = -6.0  # 降低惩罚（从-4.0）
        cfg.rewards.scales.static_legs_penalty = -3.0  # 降低惩罚（从-3.0）
        cfg.rewards.scales.continuous_contact = -0.2  # 微量惩罚，鼓励四腿同时接触
